  ...
  return (
    <div 
      ...
      className={`cursor-pointer p-2 rounded-xl shadow-sm transition-all duration-500 relative group/icon flex items-center justify-center ${
        isAnimating ? 'scale-90 rotate-12' : 'hover:scale-125 hover:-rotate-12'
      } ${activeColor} text-white ${className} ${isHovered ? 'shadow-lg shadow-indigo-500/40 ring-2 ring-white/20' : ''}`}
    >
      <div className={`relative z-10 transition-transform duration-500 ${isHovered ? 'animate-pulse' : ''}`}>
        {icon}
      </div>
      {isAnimating && (
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-50/50 dark:bg-slate-800/30 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-wrap gap-2">{result.references.map((ref, i) => (<span key={i} className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl text-[9px] font-black text-slate-500 shadow-sm">{ref}</span>))}</div>
                </div>
              </div>
            </div>
          </CollapsibleSection>

          {/* Tasks Section - Positioned before FAQ */}
          <CollapsibleSection
            id="tasks"
